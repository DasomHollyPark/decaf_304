
# parsetab.py
# This file is automatically generated. Do not edit.
_tabversion = '3.8'

_lr_method = 'LALR'

_lr_signature = 'FE173988D1DECA5F7BE5831FE3D24D16'
    
_lr_action_items = {'$end':([1,4,5,6,8,9,10,11,12,14,15,24,27,29,30,33,34,36,37,38,41,42,44,45,47,49,],[-2,-32,-33,-31,0,-4,-5,-39,-34,-3,-1,-6,-25,-38,-23,-21,-28,-26,-24,-13,-22,-27,-30,-20,-28,-29,]),'FALSE':([0,],[12,]),'SEMICOLON':([31,32,33,39,45,46,48,],[38,-18,-20,-17,-20,-18,-19,]),'INT':([0,3,7,16,18,20,27,28,30,33,43,],[-7,-12,-10,21,-8,-11,21,-9,21,21,21,]),'TRUE':([0,],[5,]),'VOID':([0,3,7,16,18,20,28,],[-7,-12,-10,22,-8,-11,-9,]),'FLOAT':([0,3,7,16,18,20,27,28,30,33,43,],[-7,-12,-10,23,-8,-11,23,-9,23,23,23,]),'PRIVATE':([0,],[7,]),'ID':([0,3,7,16,18,19,20,21,22,23,25,26,28,35,40,],[11,-12,-10,27,-8,29,-11,-14,30,-15,-16,33,-9,45,45,]),'THIS':([0,],[13,]),'BOOLEAN':([0,3,7,16,18,20,27,28,30,33,43,],[-7,-12,-10,25,-8,-11,25,-9,25,25,25,]),'STATIC':([0,7,18,],[3,20,28,]),'COMMA':([32,33,34,44,45,46,47,],[40,-20,43,-30,-20,40,43,]),'NULL':([0,],[4,]),'SUPER':([0,],[17,]),'PUBLIC':([0,],[18,]),'DOT':([2,4,5,6,9,12,13,17,],[19,-32,-33,-31,-35,-34,-36,-37,]),'STRING_CONST':([0,],[6,]),}

_lr_action = {}
for _k, _v in _lr_action_items.items():
   for _x,_y in zip(_v[0],_v[1]):
      if not _x in _lr_action:  _lr_action[_x] = {}
      _lr_action[_x][_k] = _y
del _lr_action_items

_lr_goto_items = {'field_access':([0,],[10,]),'formal_param':([27,30,33,43,],[34,34,34,47,]),'field_decl':([0,],[14,]),'variables':([26,],[31,]),'constructor_decl':([0,],[1,]),'method_decl':([0,],[15,]),'var_decl':([16,],[24,]),'primary':([0,],[2,]),'start':([0,],[8,]),'literal':([0,],[9,]),'variable':([26,35,40,],[32,44,46,]),'formal_param2':([34,47,],[42,49,]),'modifier':([0,],[16,]),'type':([16,27,30,33,43,],[26,35,35,35,35,]),'variable2':([32,46,],[39,48,]),'formals':([27,30,33,],[36,37,41,]),}

_lr_goto = {}
for _k, _v in _lr_goto_items.items():
   for _x, _y in zip(_v[0], _v[1]):
       if not _x in _lr_goto: _lr_goto[_x] = {}
       _lr_goto[_x][_k] = _y
del _lr_goto_items
_lr_productions = [
  ("S' -> start","S'",1,None,None,None),
  ('start -> method_decl','start',1,'p_start','decafparser.py',13),
  ('start -> constructor_decl','start',1,'p_start','decafparser.py',14),
  ('start -> field_decl','start',1,'p_start','decafparser.py',15),
  ('start -> literal','start',1,'p_start','decafparser.py',16),
  ('start -> field_access','start',1,'p_start','decafparser.py',17),
  ('field_decl -> modifier var_decl','field_decl',2,'p_field_decl','decafparser.py',21),
  ('modifier -> <empty>','modifier',0,'p_modifier','decafparser.py',25),
  ('modifier -> PUBLIC','modifier',1,'p_modifier','decafparser.py',26),
  ('modifier -> PUBLIC STATIC','modifier',2,'p_modifier','decafparser.py',27),
  ('modifier -> PRIVATE','modifier',1,'p_modifier','decafparser.py',28),
  ('modifier -> PRIVATE STATIC','modifier',2,'p_modifier','decafparser.py',29),
  ('modifier -> STATIC','modifier',1,'p_modifier','decafparser.py',30),
  ('var_decl -> type variables SEMICOLON','var_decl',3,'p_var_decl','decafparser.py',36),
  ('type -> INT','type',1,'p_type','decafparser.py',41),
  ('type -> FLOAT','type',1,'p_type','decafparser.py',42),
  ('type -> BOOLEAN','type',1,'p_type','decafparser.py',43),
  ('variables -> variable variable2','variables',2,'p_variables','decafparser.py',49),
  ('variable2 -> <empty>','variable2',0,'p_variable2','decafparser.py',54),
  ('variable2 -> COMMA variable variable2','variable2',3,'p_variable2','decafparser.py',55),
  ('variable -> ID','variable',1,'p_variable','decafparser.py',59),
  ('method_decl -> modifier type ID','method_decl',3,'p_method_decl','decafparser.py',70),
  ('method_decl -> modifier type ID formals','method_decl',4,'p_method_decl','decafparser.py',71),
  ('method_decl -> modifier VOID ID','method_decl',3,'p_method_decl','decafparser.py',72),
  ('method_decl -> modifier VOID ID formals','method_decl',4,'p_method_decl','decafparser.py',73),
  ('constructor_decl -> modifier ID','constructor_decl',2,'p_constructor_decl','decafparser.py',78),
  ('constructor_decl -> modifier ID formals','constructor_decl',3,'p_constructor_decl','decafparser.py',79),
  ('formals -> formal_param formal_param2','formals',2,'p_formals','decafparser.py',85),
  ('formal_param2 -> <empty>','formal_param2',0,'p_formal_param2','decafparser.py',89),
  ('formal_param2 -> COMMA formal_param formal_param2','formal_param2',3,'p_formal_param2','decafparser.py',90),
  ('formal_param -> type variable','formal_param',2,'p_formal_param','decafparser.py',94),
  ('literal -> STRING_CONST','literal',1,'p_literal','decafparser.py',103),
  ('literal -> NULL','literal',1,'p_literal','decafparser.py',104),
  ('literal -> TRUE','literal',1,'p_literal','decafparser.py',105),
  ('literal -> FALSE','literal',1,'p_literal','decafparser.py',106),
  ('primary -> literal','primary',1,'p_primary','decafparser.py',109),
  ('primary -> THIS','primary',1,'p_primary','decafparser.py',110),
  ('primary -> SUPER','primary',1,'p_primary','decafparser.py',111),
  ('field_access -> primary DOT ID','field_access',3,'p_field_access','decafparser.py',114),
  ('field_access -> ID','field_access',1,'p_field_access','decafparser.py',115),
]
